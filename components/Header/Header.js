  ...
  useEffect(() => {
    ...
    menu
      ? gsap.fromTo(
          ".rolldown",
          { y: 0 },
          { y: dropdownHeight, duration: 0.5, ease: "none" }
        )
      : gsap.to(".rolldown", { y: 0, duration: 0.5 })
  })
  ...
  return (
    <div className={styles.headercontainer}>
      ...
      <Dropdown
        className="rolldown"
        openState={menu}
        changeOpenState={(state) => setMenu(state)}
        setDropdownHeight={(height) => setDropdownHeight(height)}
      />
    </div>
  )
}
